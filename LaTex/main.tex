...
\begin{table}[H]%ti ho messo io la H maiuscola sennò la mette a fanculo
\centering
\caption{Dati sperimentali della variazione di massa dell'azoto liquido, suddivisi analiticamente nelle tre fasi sequenziali dell'esperimento.}
\vspace{0.3cm}
\footnotesize 
...
\begin{equation*}
    C_{m, \text{Grafite}} = (1.20 \pm 0.13)\text{ cal}/(\text{mol}\cdot\text{K})
\end{equation*}
...
\begin{equation*}
    C_{m, \text{Grafite}} = (1.15 \pm 0.10)\text{ cal}/(\text{mol}\cdot\text{K})
\end{equation*}


\subsection{Calcolo della Massa Evaporata (Piombo)}

Per isolare la massa di azoto evaporata esclusivamente a causa del raffreddamento del
blocco di piombo, i dati delle scale di background iniziali (Fase I) e finali (Fase III)
sono stati interpolati tramite regressione lineare. Le rette ottenute sono state successivamente
valutate in corrispondenza dell'istante mediano dell'immersione, individuato a $t_m$ = 321.835 s.
...
\begin{equation*}
    C_{m, \text{Piombo}} = (7.07 \pm 0.86)\text{ cal}/(\text{mol}\cdot\text{K})
\end{equation*}
\newpage
...
\begin{equation*}
    C_{m, \text{Piombo}} = (7.18 \pm 0.69)\text{ cal}/(\text{mol}\cdot\text{K})
\end{equation*}

\section{Conclusioni}

L'esperienza ci ha permesso di determinare il calore latente di evaporazione dell'azoto
liquido e i calori specifici medi di due campioni solidi di grafice e piombo.

Per quanto riguarda il calore latente di evaporazione dell'azoto, le due sessioni di misura hanno
fornito i valori finali di $253 \pm 19$ J/g e $172 \pm 11$ J/g.
Sebbene la prima misura si avvicini al valore di riferimento della letteratura scientifica
di circa 199 J/g, la fluttuazione tra le due sessioni suggerisce la presenza di dispersioni
termiche variabili.
La discrepanza rispetto al valore teorico può essere attribuita a una non perfetta
adiabaticità del vaso Dewar e agli scambi termici con l'ambiente.
Per minimizzare gli errori sistematici nell'analisi dei campioni, è stato razionalmente
scelto il valore della seconda calibrazione (172 J/g) per garantire la massima coerenza con le 
risposte termiche dell'apparato durante le sessioni successive.

I calori specifici molari medi, calcolati nell'intervallo termico tra 77.0 K e 293.0 K, hanno restituito
i seguenti risultati:
\begin{itemize}
    \item Grafite: I valori ottenuti nelle due sessioni, pari a $1.20 \pm 0.13 cal/(mol K)$ e $1.15 \pm 0.10 cal/(mol K)$, risultano estremamente coerenti tra loro e ben allienati con i valori teorici attesi per questo intervallo di temperatura.
    \item Piombo: Le misurazioni hanno fornito i valori di $7.07 \pm 0.86 cal/(mol K)$ e $7.18 \pm 0.69 cal/(mol K)$. Questi risultati si presentano leggermente sovrastimati rispetto al range teorico previsto, che oscilla tra 5.54 e 6.64 cal/(mol K)
\end{itemize}

Le cause di queste deviazioni, in particolare per il blocco di piombo, risiedono nelle inefficienze durante il transitorio di raffreddamento.
Il rapido trasferimento del campione a temperatura ambiente all'interno del calorimetro comporta inevitabili scambi di calore non tracciati.

In sintesi, il metodo grafico e l'estrapolazione ai minimi quadrati si sono rivelati soliti per isolare la massa evaporata netta limitando il forte background ambientale.
Tuttavia l'accuratezza assoluta delle misurazioni risente intrinsecamente delle condizioni non ideali di isolamento dell'apparato sperimentale, seppur garantendo una notevole precisione e riproducilibità statistica tra tentativi indipendenti.
\end{document}